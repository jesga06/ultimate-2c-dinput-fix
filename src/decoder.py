...
using a layout profile generated by the calibration tool.
"""
import json
import os
...
class ControllerState:
    ...
    Includes digital buttons, D-pad, analog sticks, analog triggers, and extra back buttons.
    """
    a: bool = False
    b: bool = False
    x: bool = False
...
class Decoder:
    ...
    masks and offsets stored in a JSON device profile.
    """
        ...
        if os.path.exists(profile_path):
            try:
                with open(profile_path, 'r') as f:
                    ...
                    self.extra_buttons_config = self.profile.get(
                        'extra_buttons', {})
            ...
            print(
                f"Warning: Profile {profile_path} not found. Inputs will be ignored.")
    ...
    def decode(self, data) -> ControllerState:
        ...
        if dpad_cfg and dpad_cfg.get(
                'type') == 'hat' and dpad_cfg['byte'] < len(data):
        ...
        for extra_name, cfg in self.extra_buttons_config.items():
            if cfg['byte'] < len(data):
                state.extra_buttons[extra_name] = bool(
                    data[cfg['byte']] & cfg['mask'])
