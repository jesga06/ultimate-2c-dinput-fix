...
- **Chords & Hardware Chords Master Tutorial & Info Box:** Added a prominent tutorial card to the top of the Advanced Tab and an interactive modal guide window (`open_chords_guide_modal`) detailing the differences between Hardware Chords (upstream input suppression) and Standard Chords & Macros (software macro engine), complete with step-by-step setup guides.
