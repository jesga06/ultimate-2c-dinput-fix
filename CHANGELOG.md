...
- **Console Spam Reduction:** Silenced the expected "Read failed instantly" print statements in `hid_reader.py` that occurred when gracefully ignoring locked system composite interfaces (like keyboards) during device discovery.- **Dashboard Button Flickering:** Fixed an issue where the GUI dashboard buttons would continuously redraw and flicker as gray boxes due to constant fg_color updates.
