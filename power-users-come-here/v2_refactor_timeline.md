## V2 Architectural Refactor & "Universal" Crisis
...
* We quickly learned that simple byte delta comparisons were not enough. Analog sticks constantly "shiver" and drift due to imperfect human input and jittery hardware. We implemented amplitude thresholds and multi-byte checks to ensure diagonal movement didn't cause cross-contamination.
...
* We pulled a raw transport log and realized the Machenike G5 Pro uses a "Composite HID" structure: it splits its telemetry across *multiple separate USB HID interfaces* simultaneously. Most of the standard buttons were on Interface 0, but the analog triggers and paddles were ONLY broadcasting on Interface 2.
...
* Following the deployment of the Magic Filter, the calibrator mapped the entire Machenike G5 Pro composite layout perfectly on the first try. The V2 architecture was officially proven.
